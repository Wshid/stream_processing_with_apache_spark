...
### 10.5.2. 작동
- 소켓 소스는 구성에 지정된 `TCP 서버`에 연결하는
  - `TCP 클라이언트`를 생성
- 이 클라이언트는 **스파크 드라이버**에서 실행
- 쿼리가 소비하고, 해당 오프셋이 **커밋**될 때까지
  - 들어오는 데이터를 **메모리에 유지**
- 커밋된 오프셋의 데이터가 제거되어
  - 정상적인 상황에서 **메모리 사용이 안정적으로 유지**
- 스파크 드라이버의 실패는
  - 메모리가 커밋되지 않은 모든 데이터를 **잃게 하기 때문**에
  - 이 소스는 **신뢰하지 못함**
- 이 소스는 **데이터 손실이 허용될 때만** 사용 가능

#### NOTE
- **소켓 소스**를 사용하여 `TCP 서버`에 직접 연결하는 일반적인 **아키텍처** 대안은
  - 카프카를 안정적인 **중간 저장소**로서 사용하는 것
- 강력한 **마이크로 서비스**가
  - TCP 서버와 **카프카** 사이를 메우기 위해 사용될 수 있음
이 마이크로 서비스는 `TCP 서버`에서 데이터를 수집하여
  **원자적**으로 카프카로 전달
이후 신뢰할 수 있는 **카프카 소스**를 사용하여
  데이터를 소비하고, 구조적 스트리밍에서 추가 처리 가능

## 10.6. 레이트 소스
- `records/second`로 설정된 구성 가능한 빈도로 **레코드의 시퀀스**를 생성하는 **내부 스트림 생성기**
- 출력은 `(timestamp, value)`의 스트림
- `timestamp`는 레코드의 생성 순간에 해당하며
  - `value`는 계속 증가하는 카운터
- 코드
  ```scala
...
val stream = spark.readStream.format("rate")
      ...
      .load()
```
- `60초`의 `ramp-up time`으로
  - 초당 `100행`의 레이트 스트림을 생성
- 결과 데이터 프레임 스키마에는
  - 타임스탬프 유형의 `timestamp`와
- 스키마 레벨에서는 `BigInt`이자
  - 내부적 표현으로는 `Long`인 `value`라는 두개의 필드 존재

### 10.6.1. 옵션
- 레이트 소스는 **처리량**과 **병렬 처리 레벨**을 제어하는 몇 가지 옵션 지원

#### rowsPerSecond(default: `1`)
- 초당 생성 가능한 행 수

#### rampUpTime(default: `0`)
- 스트림 시작시, 이 시간에 도달할 때까지
  - **레코드 생성이 점진적으로 증가**
- 증가는 **선형적**

#### numPartitions(default: 기본 스파크 병렬 처리 레벨)
- 생성할 **파티션 수**
- 파티션이 많을 수록
  - `레코드 생성` 및 `다운스트림 쿼리 처리의 병렬 처리 레벨`이 높아짐